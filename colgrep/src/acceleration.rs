...
pub fn is_colgrep_gpu_provider_available(provider: ExecutionProvider) -> bool {
...
pub fn available_colgrep_gpu_providers() -> Vec<ExecutionProvider> {
    next_plaid_onnx::compiled_gpu_execution_providers()
        .into_iter()
        .filter(|provider| is_colgrep_gpu_provider_available(*provider))
        .collect()
}

/// Preferred available GPU execution provider for colgrep.
pub fn preferred_colgrep_gpu_provider() -> Option<ExecutionProvider> {
    available_colgrep_gpu_providers().into_iter().next()
}

/// Whether colgrep currently has any usable GPU inference provider.
pub fn has_colgrep_gpu_provider() -> bool {
    preferred_colgrep_gpu_provider().is_some()
}

/// Require a GPU provider, returning a user-facing diagnostic if none is usable.
pub fn require_colgrep_gpu_provider() -> Result<ExecutionProvider> {
    ...
    if compiled.is_empty() {
        bail!(
            "FORCE_GPU is set, but this colgrep binary was compiled without a GPU execution provider. Enable a feature such as 'cuda', 'rocm'/'migraphx', 'coreml', or 'directml'."
        );
    }
