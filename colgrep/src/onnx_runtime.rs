...
//! When the `migraphx`/`rocm` feature is enabled, discovers an externally installed
...
/// When `migraphx`/`rocm` is enabled, discovers an installed MIGraphX-capable
...
pub fn ensure_onnx_runtime() -> Result<PathBuf> {
    ...
    // `--features rocm --force-gpu` usable without accidentally downloading the
